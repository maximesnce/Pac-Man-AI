...
def sentence1():
    ...
    a_or_b = A | B
    not_a_if = ~A  % ((~B) | C)
    not_a_or = disjoin(~A, ~B, C)

    return conjoin(a_or_b, not_a_if, not_a_or)
...
def sentence2():
    ...
    return conjoin(un, deux, trois, quatre)
    #raise NotImplementedError
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals):
    ...
    for combination in itertools.combinations(not_literals, 2) :
        test.append(combination[0] | combination[1])
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    for coord in non_outer_wall_coords :
        L.append(PropSymbolExpr(pacman_str, coord[0], coord[1], t))

    pacphysics_sentences.append(exactlyOne(L))
        

    M = []
    for action in DIRECTIONS : 
        M.append(PropSymbolExpr(action, t))

    pacphysics_sentences.append(exactlyOne(M))
...
def check_location_satisfiability(x1_y1, x0_y0, action0, action1, problem):
    ...
    KB = conjoin(KB)
    pacManT1 = PropSymbolExpr(pacman_str, x1, y1,1)
    
    model1 = findModel(conjoin(~pacManT1, KB))
    model2 = findModel(conjoin(pacManT1, KB))
...
def positionLogicPlan(problem):
    ...
    for time in range(50) :
        ...
        for x, y in non_wall_coords :
            not_in_wall.append(PropSymbolExpr(pacman_str, x , y, time))
            KB.append(pacmanSuccessorStateAxioms(x, y, time+1, walls))
        
        KB.append(exactlyOne(not_in_wall))
        model_feeder = conjoin(conjoin(KB), PropSymbolExpr(pacman_str, xg, yg, time))
        model = findModel(model_feeder)
        ...
        list_actions = []
        for action in DIRECTIONS :
            list_actions.append(PropSymbolExpr(action, time))
...
def foodLogicPlan(problem):
    ...
    for time in range(50) :
        ...
        not_in_walls = []
        for x, y in non_wall_coords :
            not_in_walls.append(PropSymbolExpr(pacman_str, x , y, time))
            #KB.append(pacmanSuccessorStateAxioms(x, y, time+1, walls))
        
        KB.append(exactlyOne(not_in_walls))

        model_feeder_1 = []
        for x, y in food :
            model_feeder_1.append(~PropSymbolExpr(food_str, x, y, time))
            KB.append((~PropSymbolExpr(food_str,x, y, time+1)) % ((~PropSymbolExpr(food_str, x, y, time)) | (PropSymbolExpr(pacman_str, x, y, time))))
            
        model_feeder = conjoin(conjoin(KB), conjoin(model_feeder_1))
        model = findModel(model_feeder)
        ...
        for x, y in non_wall_coords :
            KB.append(pacmanSLAMSuccessorStateAxioms(x, y, time+1, walls))
